...
CREATE PROCEDURE RETRIEVE_ALARMS(
    ...
    SELECT A.ID, A.RAISED_DATE, ALS.VALUE STATUS, V.NAME VARIABLE, V.UNIT VARIABLE_UNIT, I.NAME INSTRUMENT, A.MIN_VALUE, A.MAX_VALUE, B.NAME
        FROM ALARM A
...
#
SELECT A.ID, B.NAME BLOCK, B.CODE BLOCK_CODE, A.RAISED_DATE, I.NAME INSTRUMENT, A.FROM_DATE, A.TO_DATE, V.NAME VARIABLE, V.UNIT VARIABLE_UNIT, A.AVG_VALUE, A.VARIANCE_VALUE, A.MIN_VALUE, A.MAX_VALUE
FROM ALARM A
INNER JOIN INSTRUMENT I ON A.INSTRUMENT_ID = I.ID
INNER JOIN BLOCK B ON I.BLOCK_ID = B.ID
...
WHERE A.ID = 1
#
# SELECT E.CODE FROM ALARM A
#     INNER JOIN INSTRUMENT I on A.INSTRUMENT_ID = I.ID
#     INNER JOIN BLOCK B on I.BLOCK_ID = B.ID
#     INNER JOIN BLOCK_ELEMENT BE on B.ID = BE.BLOCK_ID
#     INNER JOIN ELEMENT E on BE.ELEMENT_ID = E.ID
#     INNER JOIN ELEMENT_MATERIAL EM on E.ID = EM.ELEMENT_ID
#     INNER JOIN MATERIAL_RANGE MR on A.VARIABLE_ID = MR.VARIABLE_ID
# WHERE A.ID = 1
#           AND (A.MIN_VALUE <= MR.MIN_RANGE OR A.MAX_VALUE >= MR.MAX_RANGE);
#
SELECT E.ID, E.CODE, M.NAME MATERIAL, MR.MIN_RANGE, MR.MAX_RANGE, V.UNIT, W.NAME FROM ELEMENT E
    INNER JOIN ELEMENT_MATERIAL EM on E.ID = EM.ELEMENT_ID
    INNER JOIN MATERIAL M on EM.MATERIAL_ID = M.ID
    INNER JOIN MATERIAL_RANGE MR on M.ID = MR.MATERIAL_ID
    INNER JOIN VARIABLE V on MR.VARIABLE_ID = V.ID
    ...
    INNER JOIN (SELECT * FROM ALARM WHERE ID = 2) A ON I.ID = A.INSTRUMENT_ID AND A.VARIABLE_ID = V.ID
